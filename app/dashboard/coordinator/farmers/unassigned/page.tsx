...
export default function CoordinatorUnassignedFarmersPage() {
  ...
  // For coordinator: only show unassigned farmers in their state (hardcoded to Benue for demo)
  const coordinatorState = "Benue"
  const unassignedFarmers = farmers.filter(
    (f) => f.status === "pending" && (f.assignedAgentId === "" || !f.assignedAgentId) && f.state === coordinatorState,
  )

  // Only show agents in coordinator's state
  const stateAgents = fieldAgents.filter((agent) => agent.status === "active")
    ...
    try {
      ...
      if (farmer && agent) {
        updateFarmer(selectedFarmer, {
          ...
          clusterId: agent.assignedClusterIds[0] || "",
          status: "verified",
        })
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Farmers in {coordinatorState} State who registered directly and need field agent assignment
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {unassignedFarmers.length} {unassignedFarmers.length === 1 ? "farmer" : "farmers"} waiting for assignment
            </CardDescription>
          </CardHeader>
          <CardContent>
            {unassignedFarmers.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>No unassigned farmers in your state at the moment</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                      ...
                      <TableCell>
                        <div className="flex items-start gap-1">
                          ...
                          <div className="text-sm">
                            ...
                            <div className="text-muted-foreground">{farmer.lga}</div>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        ...
                        <Dialog>
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Assign {farmer.firstName} {farmer.lastName} to one of your field agents
                              </DialogDescription>
                            </DialogHeader>
